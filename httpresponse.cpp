...
Response ResponseBuilder::build() {
    ...
    if (protocol_.empty()) throw std::runtime_error("no protocol");
    if (code_ < 0)
        throw std::runtime_error("code < 0: " + std::to_string(code_));
    if (message_.empty()) throw std::runtime_error("no message");
